...
Run manually: ``python test_full_communication.py``

This file is named ``test_*.py`` for discoverability in docs only; it is **not**
a Django unittest module. All logic runs inside ``main()`` so ``manage.py test``
does not execute network checks or call ``sys.exit`` on import.
