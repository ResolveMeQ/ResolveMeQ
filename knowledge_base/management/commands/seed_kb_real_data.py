...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i, fixture in enumerate(QUESTION_FIXTURES):
            ...
            if i % 3 == 0:
                seed_name = f"diag-{question.id}.txt"
                seed_body = (
                    f"Diagnostic log for question #{question.id}: {question.title}\n"
                    f"Generated by seed_kb_real_data for community attachment testing.\n"
                ).encode("utf-8")
                rel_path = default_storage.save(
                    f"kb_community/seed/{seed_name}",
                    ContentFile(seed_body, name=seed_name),
                )
                file_url = default_storage.url(rel_path)
                att, att_created = KBAttachment.objects.get_or_create(
                    question=question,
                    original_name=seed_name,
                    defaults={
                        "uploaded_by": author,
                        "file_path": rel_path,
                        "file_url": file_url,
                        "content_type": "text/plain",
                        "file_size": len(seed_body),
                    },
                )
                if att_created:
                    created_attachments += 1
                elif att.file_path != rel_path or att.file_url != file_url:
                    att.file_path = rel_path
                    att.file_url = file_url
                    att.file_size = len(seed_body)
                    att.save(update_fields=["file_path", "file_url", "file_size"])
